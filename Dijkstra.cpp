...
Alignment Dijkstra::run() {
  ...
  while(nodes_aligned < max_nodes){
    ...
    try{
      ...
      do {
        seed_pair = seed_mat.pop_uniform();//best_pair(seed_queue);
      } while (G1_exclude.find(seed_pair.first) != G1_exclude.end());
      //std::cout << "HERE (" << seed_pair.first << "," << seed_pair.second << ")" << std::endl;
	...
	//std::cout << "Number of NODES ALIGNED: " << nodes_aligned << " OUT OF " << max_nodes << std::endl;
      }catch (QueueEmptyException & e){
